...
function block(sub) {
  const msg =
    ...
    'search_tool/use_tool for skybox tools: index_repo, wait_for_job, query, list_repos, repo_status, impact, detect_changes ' +
...
process.stdin.on("end", () => {
  ...
  const req = http.get(
    { host: process.env.SKYBOX_DAEMON_HOST || "127.0.0.1", port: Number(process.env.SKYBOX_DAEMON_PORT || 7070), path: "/mcp", timeout: 1000 },
    (res) => {
      res.resume();
      block(sub);
    }
