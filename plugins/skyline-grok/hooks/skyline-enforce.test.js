...
// Run with: node --test plugins/skyline-grok/hooks/skyline-enforce.test.js
//
...
function assertDeniedStdout(res) {
  const line = String(res.stdout || "").trim().split(/\n/).filter(Boolean).pop() || "";
  let j;
  try { j = JSON.parse(line); } catch (_e) { assert.fail("stdout not deny JSON: " + res.stdout); }
  assert.equal(j.decision, "deny");
  assert.ok(j.reason);
}

function assertHasToolSearch(stderr) {
  assert.match(stderr, /Prefer skyline MCP tools/, "skyline tool menu present");
}
...
test("daemon-down passthrough: notice on stderr, exit 0, no block", () => {
  const res = runHook(
    "bash",
    { SKYLINE_DAEMON_PORT: "19999", GROK_SESSION_ID: "sess-down" },
    {
      tool_input: {
        command:
...
test("#706 git diff => git({subcommand:\"diff\"}) on every denial", () => {
  ...
  const r = runHook(
    "bash",
    { GROK_SESSION_ID: "sess-git-diff" },
    { tool_input: { command: "git diff | cat" } }
  );
  assert.equal(r.status, 0, "denied");
...
test("#706 Read inside tree => deny with read({path})", () => {
  ...
  const r = runHook(
    "read",
    { GROK_SESSION_ID: "sess-read-in", CLAUDE_PROJECT_DIR: path.resolve(__dirname, "../../..") },
    {
...
test("#706 Write outside tree (e.g. ~/.claude) => pass through exit 0", () => {
  ...
  const r = runHook(
    "edit",
    { GROK_SESSION_ID: "sess-write-out", CLAUDE_PROJECT_DIR: path.resolve(__dirname, "../../..") },
    {
      cwd: path.resolve(__dirname, "../../.."),
      tool_name: "Write",
...
test("#706 Write inside tree => deny with create({path})", () => {
  ...
  const r = runHook(
    "edit",
    { GROK_SESSION_ID: "sess-write-in", CLAUDE_PROJECT_DIR: path.resolve(__dirname, "../../..") },
    {
      cwd: path.resolve(__dirname, "../../.."),
      tool_name: "Write",
  ...
  assert.equal(r.status, 0, "in-tree Write denied");
...
test("#706 second denial still carries substitute (idempotent, no dead one-liner)", () => {
  ...
  const r1 = runHook("bash", { GROK_SESSION_ID: sess }, cmd);
  const r2 = runHook("bash", { GROK_SESSION_ID: sess }, cmd);
...
test("#706 ToolSearch select string unconditional on read deny", () => {
  ...
  const r = runHook(
    "read",
    {
      GROK_SESSION_ID: "sess-toolsearch",
...
test("native Grep redirect with `use App\\Models\\User` in a composer cwd: message contains `symbol_card`", () => {
  ...
  try {
    ...
    assert.equal(r.status, 0, "still denies (redirects)");
...
test("#706 CLAUDE_PROJECT_DIR unset: .git walk scopes, in-tree Read still denied", () => {
  ...
  const r = runHook(
    "read",
    {
      GROK_SESSION_ID: "sess-env-unset",
  ...
  assert.equal(r.status, 0, "env-unset in-tree Read denied via .git walk");
  assert.match(r.stderr, /read\(\{path:/);
  assertHasToolSearch(r.stderr);
});
...
test("#706 find -name => find({glob,path})", () => {
  const r = runHook(
    "bash",
    { GROK_SESSION_ID: "sess-find-name" },
...
test("#411 ls remediation renders an absolute path, never a relative '.'", () => {
  const r = runHook(
    "bash",
    { GROK_SESSION_ID: "sess-ls-abs", CLAUDE_PROJECT_DIR: path.resolve(__dirname, "../../..") },
...
test("#706 Glob => find({glob})", () => {
  const r = runHook(
    "glob",
    { GROK_SESSION_ID: "sess-glob-mode" },
...
test("#706 Edit inside tree => read-then-edit flow (edit has no path param)", () => {
  ...
  assert.equal(r.status, 0, "in-tree Edit denied");
...
test("grep flag values are not taken as the pattern", () => {
  const r1 = runHook(
    "bash",
    { GROK_SESSION_ID: "sess-grep-flagval" },
...
for (const [label, command] of LIFECYCLE_CASES) {
  test(`daemon lifecycle passes through: ${label}`, () => {
    ...
    const r = runHook(
      "bash",
      { GROK_SESSION_ID: "sess-lifecycle" },
      { tool_input: { command } }
    );
...
test("lifecycle exemption is narrow: daemon status still redirects", () => {
  ...
  const r = runHook(
    "bash",
    { GROK_SESSION_ID: "sess-lifecycle-narrow" },
    { tool_input: { command } }
  );
  assert.equal(r.status, 0, "status is a read, not lifecycle: still denied");
  assertHasToolSearch(r.stderr);
});
...
test("lifecycle exemption is narrow: grepping for the phrase still redirects", () => {
  ...
  const r = runHook(
    "bash",
    { GROK_SESSION_ID: "sess-lifecycle-grep" },
    {
      tool_input: {
        command:
  ...
  assert.equal(r.status, 0, "grep for the phrase is an ordinary search: denied");
...
test("daemon-down passthrough holds for a lifecycle command too", () => {
  ...
  const r = runHook(
    "bash",
    { SKYLINE_DAEMON_PORT: "19999", GROK_SESSION_ID: "sess-lifecycle-down" },
    {
      tool_input: {
        command:
