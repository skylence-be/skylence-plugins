...
// Grok port of skyline-claude enforce. Invoked as
// `node skyline-enforce.js <mode>`. PreToolUse: stdout JSON {decision, reason?}.
//
// Hardening lineage:
...
//     silently WRONG, not merely redirected. Measured live on the claude twin:
//     an agent asked to confirm its own HERDR_* vars via `env | grep` got zero
//     matches through `run` while `ps eww` on the same PID showed five. Ported
//     here for parity; mirrors the daemon lifecycle pass-through below.
//
...
function projectRoot(input) {
  const envRoot = process.env.GROK_WORKSPACE_ROOT || process.env.CLAUDE_PROJECT_DIR || process.env.SKYLINE_PROJECT_ROOT;
...
function filePathFromInput(mode, ti) {
  if (mode === "read" || mode === "edit") {
    return ti.target_file || ti.file_path || ti.path || ti.filePath || null;
  }
  return null;
}
...
async function main() {
  ...
  const nativeLabel =
    MODE === "bash"
      ? `run_terminal_command \`${command.length > 80 ? command.slice(0, 77) + "..." : command}\``
      : MODE === "read"
        ? "read_file"
        : MODE === "edit"
          ? String(toolName || "search_replace/write")
          : MODE === "grep"
            ? "grep"
            : MODE === "glob"
              ? "list_dir"
  ...
  process.stdout.write(
    JSON.stringify({ decision: "deny", reason: outMsg }) + "\n"
  );
  process.exit(0);
}
