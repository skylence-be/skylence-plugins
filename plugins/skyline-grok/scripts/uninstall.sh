...
# End stale plugin watchdogs before plugin cache removal.
pgrep -f '/skyline-(claude|codex|grok)/.*/monitors/' 2>/dev/null | while read -r pid; do kill "$pid" 2>/dev/null || true; done

# Remove the Grok plugin via the grok CLI.
if command -v grok >/dev/null 2>&1; then
  grok plugin uninstall skyline-grok 2>/dev/null || true
fi

# Remove the Antigravity plugin if present (cross-clean).
if command -v agy >/dev/null 2>&1; then
  agy plugin uninstall skyline-antigravity 2>/dev/null || true
fi
