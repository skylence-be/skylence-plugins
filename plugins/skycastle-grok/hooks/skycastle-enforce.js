// Grok port. PreToolUse: stdout JSON {decision, reason?}.
...
// skyline's run, so the skycastle CLI would arrive in its argv). skyline
// v1.1.0 dropped the `skyline_` prefix and split the batch/background modes
// into their own tools, so the matcher must cover all three names.
//
...
function block(sub) {
  const msg =
    ...
    'search_tool/use_tool for skycastle tools: secret_get, secret_set, secret_list, secret_delete ' +
    "then retry via the MCP tool.";
  process.stderr.write(msg + "\n");
  process.stdout.write(
    JSON.stringify({ decision: "deny", reason: msg }) + "\n"
  );
  process.exit(0);
}
...
process.stdin.on("end", () => {
  ...
  const req = http.get(
    { host: process.env.SKYCASTLE_DAEMON_HOST || "127.0.0.1", port: Number(process.env.SKYCASTLE_DAEMON_PORT || 8210), path: "/mcp", timeout: 1000 },
    (res) => {
      res.resume();
      block(sub);
    }
