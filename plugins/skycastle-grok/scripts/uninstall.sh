...
# Uninstall the skycastle-grok plugin.
#
...
# Remove the Grok plugin registration.
if command -v grok >/dev/null 2>&1; then
  grok plugin uninstall skycastle-grok 2>/dev/null || true
fi

# Also clean Antigravity sibling registration if present.
if command -v agy >/dev/null 2>&1; then
  agy plugin uninstall skycastle-antigravity 2>/dev/null || true
fi

# Verification.
echo "skycastle-grok uninstall: MCP wiring removed; skycastle install left intact."
