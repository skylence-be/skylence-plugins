...
description: Uninstall the skycastle-grok plugin — removes the skycastle MCP wiring from the host. Leaves the skycastle daemon, binary, and vault data intact (manage those with `skycastle` directly). Use when the user wants to remove the skycastle plugin.
---
...
if [ -n "$ROOT" ] && [ -f "$ROOT/scripts/uninstall.sh" ]; then
  ...
  grok plugin uninstall skycastle-grok 2>/dev/null || true
fi
```
