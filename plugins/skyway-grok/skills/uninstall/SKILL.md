...
description: Uninstall the skyway-grok plugin — removes the skyway MCP wiring from the host. Leaves the skyway daemon, binary, and data intact (manage those with `skyway` directly). Use when the user wants to remove the skyway plugin.
---
...
if [ -n "$ROOT" ] && [ -f "$ROOT/scripts/uninstall.sh" ]; then
  ...
  grok plugin uninstall skyway-grok 2>/dev/null || true
fi
```
