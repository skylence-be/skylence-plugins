...
STATUS / BUDGET (every run spends real $)
...
RULES: report run state from tool output, never assumption. Surface cost before large runs. Down daemon ⇒ wait + retry, not native fallback.
